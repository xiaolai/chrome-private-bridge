...
describe("ChromeBridge client", () => {
  ...
  test("copyToClipboard() with imagePath sends correct command", async () => {
    const result = await bridge.copyToClipboard({ imagePath: "/tmp/test.png" })
    expect(result.ok).toBe(true)
    const body = await lastRequest!.json()
    expect(body.command).toBe("os.clipboard.write")
    expect(body.params.imagePath).toBe("/tmp/test.png")
  })

  test("copyToClipboard() with html sends correct command", async () => {
    const result = await bridge.copyToClipboard({ html: "<p>hi</p>" })
    expect(result.ok).toBe(true)
    const body = await lastRequest!.json()
    expect(body.command).toBe("os.clipboard.write")
    expect(body.params.html).toBe("<p>hi</p>")
  })

  test("paste() sends correct command with defaults", async () => {
    const result = await bridge.paste()
    expect(result.ok).toBe(true)
    const body = await lastRequest!.json()
    expect(body.command).toBe("os.paste")
  })

  test("paste() with options sends correct params", async () => {
    const result = await bridge.paste({ retries: 5, delay: 200, app: "Chrome" })
    expect(result.ok).toBe(true)
    const body = await lastRequest!.json()
    expect(body.command).toBe("os.paste")
    expect(body.params.retries).toBe(5)
    expect(body.params.delay).toBe(200)
    expect(body.params.app).toBe("Chrome")
  })
