...
// Set CONFIG_DIR before importing modules so the static import uses temp dir
...
describe("auth module", () => {
  beforeEach(() => {
    // Write a clean keys file for each test
  ...
  test("validateKey() with IP allowlist — allowed IP passes", () => {
    ...
    saveKeys(keys)
    // Need to ensure cache is refreshed — generate another key to invalidate
    generateKey("dummy")
  ...
  test("validateKey() with IP allowlist — blocked IP fails", () => {
    ...
    keys.keys.find(k => k.key === key)!.allowedIPs = ["10.0.0.1"]
    saveKeys(keys)
    generateKey("dummy") // invalidate cache
  ...
  test("getKeyPermissions() returns null for unrestricted key", () => {
    const key = generateKey("test")
    validateKey(key, "127.0.0.1") // populates cache
  ...
  test("flushKeys() writes dirty cache to disk", () => {
    const key = generateKey("test")
    validateKey(key, "127.0.0.1") // marks dirty
    flushKeys()
    const keys = loadKeys()
    expect(keys.keys.find(k => k.key === key)!.lastUsed).not.toBeNull()
  })
  ...
  test("flushKeys() does nothing when not dirty", () => {
    flushKeys() // should not throw or error
  })
  ...
  test("getExtensionToken() returns consistent value (singleton)", () => {
    const t1 = getExtensionToken()
    const t2 = getExtensionToken()
    expect(t1).toBe(t2)
  ...
  test("validateExtensionToken() returns true for correct token", () => {
    ...
    expect(validateExtensionToken(token)).toBe(true)
  })

  test("validateExtensionToken() returns false for wrong token", () => {
    expect(validateExtensionToken("wrong")).toBe(false)
  })
})
