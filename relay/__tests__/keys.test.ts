...
const testDir = mkdtempSync(join(tmpdir(), "chrome-bridge-keys-test-"))
...
describe("keys route handler", () => {
  beforeEach(() => {
    // Reset keys file for each test
    saveKeys({ keys: [] })
  })
  ...
  test("POST with invalid JSON body falls back gracefully", async () => {
    ...
    const resp = await handleKeys(req, "127.0.0.1")
    // body parsing catches error, action becomes null from query → unknown action
    expect(resp.status).toBe(400)
  })
})
