...
describe("config module", () => {
  ...
  test("default values when no env vars set", async () => {
    ...
    delete process.env.REST_ENABLED
    ...
    expect(config.restEnabled).toBe(true)
  })
  ...
  test("MCP_ENABLED=false disables MCP", async () => {
    process.env.MCP_ENABLED = "false"
    const { config } = await import("../config")
    expect(config.mcpEnabled).toBe(false)
  })

  test("REST_ENABLED=false disables REST", async () => {
