...
describe("auth module", () => {
  ...
  test("generateKey() with allowedIPs", () => {
    const key = generateKey("ip-restricted", null, ["10.0.0.1", "10.0.0.2"])
    const keys = loadKeys()
    const entry = keys.keys.find(k => k.key === key)
    expect(entry!.allowedIPs).toEqual(["10.0.0.1", "10.0.0.2"])
  })
  ...
  test("hasKeys() returns false when no keys exist", () => {
    saveKeys({ keys: [] })
    expect(hasKeys()).toBe(false)
  })

  test("hasKeys() returns true after generating a key", () => {
    saveKeys({ keys: [] })
    generateKey("test-haskeys")
    expect(hasKeys()).toBe(true)
  })

  test("hasKeys() returns true when store is corrupted (fail closed)", () => {
    // Corrupt the file and invalidate cache by revoking a nonexistent key
    writeFileSync(join(process.env.CONFIG_DIR!, "keys.json"), "corrupted{{{")
    revokeKey("bby_zzzz") // invalidates keyCache, loadKeys reads corrupted file
    expect(hasKeys()).toBe(true)
  })
