...
export async function handleCommand(req: Request, token: string | null): Promise<Response> {
  ...
  if (!def) {
    return json({ ok: false, error: `Unknown command: "${body.command}"` }, 400)
  }
...
function checkPermissions(command: string, token: string | null): Response | null {
  if (!token) return null
  const allowed = getKeyPermissions(token)
  if (!allowed) return null // unrestricted key

  // Check raw command string, MCP tool name, and extension command
  if (allowed.includes(command)) return null
  const def = getCommand(command) ?? findByExtensionCommand(command)
  if (def && (allowed.includes(def.name) || allowed.includes(def.extensionCommand))) {
    return null
  }
...
function validateParams(
  def: NonNullable<ReturnType<typeof getCommand>>,
...
async function executeCommand(
  ...
  const keyPrefix = token ? token.slice(0, 8) : "noauth"
  const extCmd = def.extensionCommand
