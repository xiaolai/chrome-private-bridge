...
describe("ws/manager", () => {
  ...
  test("handleMessage() rejects JSON primitives (not objects)", () => {
    const ws = createMockWs()
    handleOpen(ws)
    handleMessage(ws, "42")
    expect(JSON.parse(ws.sentMessages[0]).error).toBe("Expected JSON object")
    ws.sentMessages.length = 0

    handleMessage(ws, "null")
    expect(JSON.parse(ws.sentMessages[0]).error).toBe("Expected JSON object")
    ws.sentMessages.length = 0

    handleMessage(ws, '"hello"')
    expect(JSON.parse(ws.sentMessages[0]).error).toBe("Expected JSON object")
    ws.sentMessages.length = 0

    handleMessage(ws, "[1,2,3]")
    expect(JSON.parse(ws.sentMessages[0]).error).toBe("Expected JSON object")
    handleClose(ws)
  })
