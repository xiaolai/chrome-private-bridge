import { loadKeys, saveKeys, isStoreCorrupted, type KeyStore } from "./store"
...
export function generateKey(
  name: string,
  allowedCommands: string[] | null = null,
...
export function hasKeys(): boolean {
  if (!keyCache) keyCache = loadKeys()
  if (isStoreCorrupted()) return true // fail closed: require auth when store is corrupted
  return keyCache.keys.length > 0
}
